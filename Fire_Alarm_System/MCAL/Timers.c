...
/************************* TIMER0__Pointer to functions to be assigned to ISR*****************/

static void (*Timer0_OV_Fptr) (void)=NULLPTR; //Normal Mode
static void (*Timer0_OC_Fptr) (void)=NULLPTR;

/************************* TIMER1__Pointer to functions to be assigned to ISR*****************/

static void (*Timer1_OVF_Fptr) (void)=NULLPTR; //Normal Mode
...
/************************* TIMER2__Pointer to functions to be assigned to ISR*****************/

static void (*Timer2_OV_Fptr) (void)=NULLPTR; //Normal Mode
...
void TIMER0_Init(Timer0Mode_type mode,Timer0Scaler_type scaler,OC0Mode_type OC_mode)
{
	...
	switch (OC_mode)
	{
		case OC0_DISCONNECTED: //NOTE:By Default is disconnected.
		CLR_BIT(TCCR0,COM00);
		CLR_BIT(TCCR0,COM01);
		break;
		...
		CLR_BIT(TCCR0,COM01);
		break;
		case OC0_NON_INVERTING:  // Clear OC0 on compare match, set OC0 at BOTTOM,(non-inverting mode).
		CLR_BIT(TCCR0,COM00);
		SET_BIT(TCCR0,COM01);
		break;

		case OC0_INVERTING:      // Set OC0 on compare match, clear OC0 at BOTTOM,(inverting mode).
		SET_BIT(TCCR0,COM00);
		SET_BIT(TCCR0,COM01);
		break;
	}
...
void TIMER2_Init(Timer2Mode_type mode,Timer2Scaler_type scaler,OC2Mode_type OC_mode)
{
	...
	switch (OC_mode)
	{
		case OC2_DISCONNECTED: //NOTE:By Default is disconnected.
		CLR_BIT(TCCR2,COM20);
		CLR_BIT(TCCR2,COM21);
		break;
		...
		CLR_BIT(TCCR2,COM21);
		break;
		case OC2_NON_INVERTING:  // Clear OC0 on compare match, set OC0 at BOTTOM,(non-inverting mode).
		CLR_BIT(TCCR2,COM20);
		SET_BIT(TCCR2,COM21);
		break;

		case OC2_INVERTING:      // Set OC0 on compare match, clear OC0 at BOTTOM,(inverting mode).
		SET_BIT(TCCR2,COM20);
		SET_BIT(TCCR2,COM21);
		break;
	}
